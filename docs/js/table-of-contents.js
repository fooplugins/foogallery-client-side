(function($){
	 ...
	 * @constructor TOC
	 * @example {@caption The following shows the ready callback that is registered when this script is loaded.}
	 * $(function(){
	 *
	 * 	var toc = new TOC();
	 * 	toc.parse();
	 ...
	 */
	function TOC(){
		if (!(this instanceof TOC)) return new TOC();
	 ...
	 *
	 * Also registers the header using the {@link TOC#add} method.
	...
	TOC.prototype.parse = function(){
		var self = this;
		$('.main-content').find('h1,h2,h3,h4').not('.toc-ignore').each(function(){
			var $header = $(this), id = $header.attr('id'), html = self.getHTML($header);
			if (!id){
				id = self.slugify($($.parseHTML(html)).text());
				$header.attr('id', id);
			}
			$header.addClass('anchored').append(self.$anchorLink(id));
			self.add($header, html);
		});
	};
	...
	TOC.prototype.getHTML = function($header){
		var html = $header.clone()
			.find('.anchor-link,small,code,button,.toc-ignore').remove()
			.end()
			.get(0).innerHTML;
	 ...
	 * @param {string} html - The HTML to display for the item.
	 * @returns {TOC~Item} The item created from the header is returned.
	 */
	TOC.prototype.add = function($header, html){
		var item = {
			id: $header.prop('id'),
			$header: $header,
			header: $header.get(0),
	...
	TOC.prototype.$nav = function(items){
		...
		for (var i = 0, l = items.length; i < l; i++){
			items[i].$li = $('<li/>', {'class': 'toc-'+items[i].tag});
	...
	/**
	 * This method initializes the plugin generating the items, appending them to the DOM and initializes the scroll spy.
	...
	/**
	 * When no items were registered this can be called to fix the column layout removing the space reserved for the TOC.
