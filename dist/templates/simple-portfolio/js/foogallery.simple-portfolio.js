(function($, _, _utils, _is){
	...
	_.SimplePortfolio = _utils.Class.extend({
		construct: function(element, options){
			this.$el = $(element);
			this.options = $.extend(true, {}, _.SimplePortfolio.defaults, options);
			this._items = [];
		},
		init: function(){
			var self = this;
			self._items = self.$el.find(".foogallery-item").map(function(i, el){
				var $item = $(el),
					$thumb = $item.find(".foogallery-thumb"),
					$img = $thumb.find("img"),
			...
			$(window).on("resize.simple-portfolio", {self: self}, self.onwindowresize);
			self.layout();
		},
		destroy: function(){
			$(window).off("resize.simple-portfolio");
		},
	...
	$.fn.fgSimplePortfolio = function(options){
		return this.each(function(){
			var fb = $.data(this, "__FooGallerySimplePortfolio__");
			if (fb){
				fb.destroy();
			}
			fb = new _.SimplePortfolio(this, options);
			fb.init();
			$.data(this, "__FooGallerySimplePortfolio__", fb);
		});
	};
